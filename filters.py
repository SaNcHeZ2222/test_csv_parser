...
def apply_filter(rows: List[Dict[str, str]], condition: str) -> List[Dict[str, str]]:
    ...
    if '==' in condition:
        ...
        try:
            ...
            raise ValueError(f"Filter value '{val}' for column '{col}' must be a number")  # Use 'val' here
        return [row for row in rows if is_float(row[col], col) and float(row[col]) > val_float]
